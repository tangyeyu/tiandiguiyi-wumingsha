			...
			game.import('character', function () {
				...
				pkg = {
					...
					translate: {
						...
						'zycf_extra': '摧锋',
						'zycf_extra_info': '1.每轮限一次，每回合结束时，你可以摸一张牌并执行一个额外的回合（由你选择是否发动；额外回合包含判定/摸牌阶段，该额外回合结束时不能再发动本项）。<br>2.每回合结束时，你可以从装备栏或手牌中选择一张装备弃置，然后你可以视为对当前回合角色使用一张伤害值+X的【决斗】（X为本回合所有角色打出【杀】的次数，同一张【杀】指定多个目标按1次计）。该【决斗】不能以自己为目标，且至少造成1点伤害（若被【无懈可击】取消则保底不成立）。',
						// ★ 技能描述＝用户提供的原文，不再改写
