			...
			game.import('character', function () {
				...
				pkg = {
					...
					translate: {
						...
						'zyyi': '武翊',
						// ★ 技能描述＝用户提供的原文，不再改写
						'zyyi_info': '1.你可以将一张【杀】当【闪】、【闪】当【杀】使用或打出。<br>2.回合开始时，你从牌堆/弃牌堆里选择一项获得：【1】进攻马、【2】防御马、【3】武器、【4】防具（先从牌堆里找，没有的话就去弃牌堆里找；两处都没有则不获得）。<br>3.你的手牌上限+x（x为你装备区内已装备的数量）。<br>4.当你装备区里有武器时，你造成伤害时，可以选择弃置一张装备栏里的武器牌或手牌中的武器牌，然后此伤害+1（同一回合内不能多次发动；同一次伤害不能叠加）。<br>5.当你装备区里有防具时，你受到伤害时，可以选择弃置一张装备栏里的防具牌或手牌中的防具牌，然后完全抵消此伤害。<br>6.当你装备区里有进攻马时，你免疫第一次受到伤害，然后你可以选择弃置装备栏里的进攻马；当你装备栏里的进攻马离开你的装备栏时，你摸两张牌（当你失去此进攻马并重新装备进攻马时，此效果刷新）。<br>7.当你装备区里有防御马时，你第一次造成的伤害+1，然后你可以选择弃置装备栏里的防御马；当你装备栏里的防御马离开你的装备栏时，你摸两张牌（当你失去此防御马并重新装备防御马时，此效果刷新）。<br>8.若你从手牌中使用或者打出过的基本牌牌名数（火杀/雷杀不计为独立牌名）大于等于1/2/3：你使用基本牌无次数限制／你使用基本牌无距离限制／当你获得或失去装备时，你摸一张牌。',
						'zycf': '摧锋',
						'zycf_info': '1.每轮限一次，每回合结束时，你可以摸一张牌并执行一个额外的回合（额外回合包含判定/摸牌阶段；该额外回合结束时不能再发动本项）。<br>2.每回合结束时，你可以从装备栏或手牌中选择一张装备弃置，然后你可以视为对当前回合角色使用一张伤害值+X的【决斗】（X为本回合所有角色使用或打出【杀】的次数，至多为3；同一张【杀】指定多个目标按1次计）。该【决斗】不能以自己为目标，且至少造成1点伤害（若被【无懈可击】取消则保底不成立）。',
