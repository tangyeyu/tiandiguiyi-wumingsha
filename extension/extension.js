			...
			game.import('character', function () {
				...
				pkg = {
					...
					translate: {
						...
						'zycf_extra': '摧锋',
						'zycf_extra_info': '1.每轮限一次，每回合结束时，你可以摸一张牌并执行一个额外的回合（由你选择是否发动；额外回合包含判定/摸牌阶段，该额外回合结束时不能再发动本项）。<br>2.每回合结束时，你可以从装备栏或手牌中选择一张装备弃置，然后你可以视为对当前回合角色使用一张伤害值为1+X的【决斗】（X为本回合所有角色打出【杀】的次数；同一张【杀】指定多个目标按1次计）。该【决斗】不能以自己为目标，且至少造成1点伤害（若被【无懈可击】取消则保底不成立）。',
						// ★ 技能描述＝用户提供的原文，不再改写
					...
					skill: {
						...
						zl_xing_tu: {
							charlotte: true, sub: true, popup: false,
							marktext: '星',
							// ★★ 「星」存放于 **expansions 独立容器**（'x' 区）★★
							//   为什么不用 's' 区（原版观星用的那个）：
							//   's' 区在引擎里是 directgains 实现的（game.js:25989-25993）：
							//       cards[i].classList.add('glows');
							//       node.appendChild(cards[i]) // → this.node.handcards1
							//   ⇒ 牌被塞进**手牌容器**，引擎把它当手牌
							//   ⇒ 显示在手牌区、还能被直接点用
							//     （用户实测"星显示在手牌区且可以直接用"）。
							//   addToExpansion 放进 player.node.expansions：独立容器、
							//   引擎自带渲染（看得见）、但不进手牌区 ⇒ 不会进通用出牌池。
							//   显示用 markcount:'expansion'（引擎自动数 'x' 区带该 gaintag 的牌）。
							intro: {
								markcount: 'expansion',
								...
								onunmark: function (storage, player) {
									var cs = lib.skill.zl_xing_tu.stars(player);
									if (cs.length) player.loseToDiscardpile(cs);
								},
							},
							// ── 统一的「星」读写（永远返回数组）──
							...
							// 把牌放进「星」区（独立容器）
							add: function (player, cards) {
								try {
									if (!cards || !cards.length) return;
									if (!player.hasSkill('zl_xing_tu')) player.addSkill('zl_xing_tu');
									var ok = player.addToExpansion(cards, player, 'zl_xing');
									if (ok && ok.gaintag) ok.gaintag.add('zl_xing');
						...
						zl_qixing: {
							audio: 'guanxing', locked: true, forced: true, charlotte: true, popup: false, direct: true,
						...
						//     玩家可以拒绝 ⇒ 不会强制弃星）
						zl_kongcheng: {
							audio: 'zl_kongcheng', charlotte: true, popup: true,
						...
						zl_kc_collect: {
							...
							content: function () {
								...
								try {
									// 该牌结算结束后若仍在弃牌堆 ⇒ 置为「星」（统一走 's' 区）
									var inPile = !!(card && card.parentNode === ui.discardPile);
									if (inPile) {
										lib.skill.zl_xing_tu.add(player, [card]);
						...
						cc_zhilue: {
							...
							content: function () {
								'step 0'
								event.pool = player.getCards('x', function (card) {
									return card.hasGaintag && card.hasGaintag('cc_zhi');
								});
								event.idx = 0;
								...
								// forceTarget = true：需要目标时由玩家选择目标
								try {
									player.chooseUseTarget(event.card,
										'志略：按记录顺序使用' + get.translation(event.card) + '（无距离、次数限制）', true, false);
								} catch (eU) { }
								'step 2'
								event.goto(1);
							},
						...
						zycf_duel_buff: {
							...
							content: function () {
								...
								//   用户口径：只统计**打出**的【杀】（响应南蛮、决斗拼杀等），
								//   不算"使用"的杀（例如别人主动杀你）⇒ 到最后一刻统一统计。
					...
					// 三将已从本扩展移除，故此表为空的占位；将来加回武将时在此登记。
					dynamicTranslate: {},
				};
				return pkg;
			});
